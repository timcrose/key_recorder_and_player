...
def mouseDown_fast(button=PRIMARY):
    ev = None
    if button == PRIMARY or button == LEFT:
        ev = MOUSEEVENTF_LEFTDOWN
    elif button == MIDDLE:
    ...
    for i in range(num_tries):
        if SendInput(1, ctypes.pointer(x), ctypes.sizeof(x)) == 1:
            return
    raise Exception('Could not mouseDown in', num_tries, 'tries.')
...
def mouseUp_fast(button=PRIMARY):
    ev = None
    if button == PRIMARY or button == LEFT:
        ev = MOUSEEVENTF_LEFTUP
    elif button == MIDDLE:
    ...
    for i in range(num_tries):
        if SendInput(1, ctypes.pointer(x), ctypes.sizeof(x)) == 1:
            return
    raise Exception('Could not mouseUp in', num_tries, 'tries.')
...
def click_fast(x=None, y=None, clicks=1, interval=0.0, button=PRIMARY, move_speed=0.0, duration=0.0, after_move_delay=0.0):
    if not x is None or not y is None:
        ait.mouse_move(x, y, move_speed)
        time.sleep(after_move_delay)
    for i in range(clicks):
        mouseDown_fast(button=button)
        time.sleep(duration)
        mouseUp_fast(button=button)
        time.sleep(interval)
...
def moveTo_fast(x=None, y=None):
    ...
    for _ in range(num_tries):
        if SendInput(1, ctypes.pointer(command), ctypes.sizeof(command)) == 1:
            return
    raise Exception('Could not moveTo in', num_tries, 'tries.')
...
def moveRel_fast(xOffset=0, yOffset=0, loops=1, return_delay=0, interval_delay=0.033):
    # When using MOUSEEVENTF_MOVE for relative movement the results may be inconsistent.
    # "Relative mouse motion is subject to the effects of the mouse speed and the two-mouse threshold values. A user
    # sets these three values with the Pointer Speed slider of the Control Panel's Mouse Properties sheet. You can 
    # obtain and set these values using the SystemParametersInfo function." 
    ...
    for _ in range(loops):
        ...
        for i in range(num_tries):
            if SendInput(1, ctypes.pointer(command), ctypes.sizeof(command)) == 1:
                ...
                break
        if not sent:
            raise Exception('Could not move mouse in', num_tries, 'tries.')
        time.sleep(interval_delay)
    time.sleep(return_delay)
    
    
def moveRel_x_then_y(xOffset=0, yOffset=0, loops=1, return_delay=0, interval_delay=0.033):
    if xOffset != 0:
        moveRel_fast(xOffset=xOffset, yOffset=0, loops=loops, return_delay=interval_delay, interval_delay=interval_delay)
    if yOffset != 0:
        moveRel_fast(xOffset=0, yOffset=yOffset, loops=loops, return_delay=interval_delay, interval_delay=interval_delay)
    time.sleep(return_delay)
...
def keyDown_fast(key):
    ...
    for _ in range(num_tries):
        if SendInput(1, ctypes.pointer(x), ctypes.sizeof(x)) == 1:
            return
    raise Exception('Could not keyDown with key =', key, 'in', num_tries, 'tries.')
...
def keyUp_fast(key):
    ...
    for _ in range(num_tries):
        if SendInput(1, ctypes.pointer(x), ctypes.sizeof(x)) == 1:
            return
    raise Exception('Could not keyUp with key =', key, 'in', num_tries, 'tries.')
...
def press_key_fast(key, duration=0.0, presses=1):
...
def typewrite_fast(message, interval=0.0, loops=1):
    interval = float(interval)
